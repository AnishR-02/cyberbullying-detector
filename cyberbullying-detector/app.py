...
for role, message in st.session_state.messages:
    if role == "user":
        ...
        score = float(match.group(1)) / 100 if match else 0

        # Color levels
        if score > 0.9:
            bg_color = "#5c1a1a"  # dark red
        elif score > 0.75:
            bg_color = "#7a3e1d"  # orange
        elif score > 0.6:
            bg_color = "#7a6a1d"  # yellow
        else:
            bg_color = "#1e3a2f"  # green

        st.markdown(
            f"""
            <div style='text-align: left; background-color: {bg_color}; padding: 10px; border-radius: 10px; margin: 5px;'>
                <b>AI:</b> {message}
            </div>
            """,
            unsafe_allow_html=True
